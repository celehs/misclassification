...
parse <- matrix(0, ncol = rep.proc, nrow = 1 + 5 + 5 + 5)
...
for (ll in 1:rep.proc) {
  ...
  parre <- apply(I, 2, resam2, initial, X, t0, d0)
...
######## cluster
write.table(cbind(c(code)), paste(getwd(), "/", "2code.csv", sep = ""), append = TRUE, col.names = FALSE, row.names = FALSE)
write.table(cbind(c(1111, parhat)), paste(getwd(), "/", "2parhat.csv", sep = ""), append = TRUE, col.names = FALSE, row.names = FALSE)
write.table(cbind(c(1111, auchat)), paste(getwd(), "/", "2auchat.csv", sep = ""), append = TRUE, col.names = FALSE, row.names = FALSE)
...
write.table(cbind(c(convergeno)), paste(getwd(), "/", "2convergeno.csv", sep = ""), append = TRUE, col.names = FALSE, row.names = FALSE)
write.table(cbind(c(1111, parse)), paste(getwd(), "/", "2parse.csv", sep = ""), append = TRUE, col.names = FALSE, row.names = FALSE)
write.table(cbind(c(code.m)), paste(getwd(), "/", "2codem.csv", sep = ""), append = TRUE, col.names = FALSE, row.names = FALSE)
write.table(cbind(c(1111, parhat.m)), paste(getwd(), "/", "2parhatm.csv", sep = ""), append = TRUE, col.names = FALSE, row.names = FALSE)

######## results
rm(list = ls())
library(readr)
code <- read_csv("/Users/xuanwang/Desktop/O2_Cluster/2code.csv")
code <- code[[1]]
parhat <- read_csv("/Users/xuanwang/Desktop/O2_Cluster/2parhat.csv")
par <- parhat[[1]]
auchat <- read_csv("/Users/xuanwang/Desktop/O2_Cluster/2auchat.csv")
auc <- auchat[[1]]
# tprhat <- read_csv("/Users/xuanwang/Desktop/O2_Cluster/tprhat.csv"); tpr=tprhat[[1]]
# fprhat <- read_csv("/Users/xuanwang/Desktop/O2_Cluster/fprhat.csv"); fpr=fprhat[[1]]
convergeno <- read_csv("/Users/xuanwang/Desktop/O2_Cluster/2convergeno.csv")
convergeno <- convergeno[[1]]
parse <- read_csv("/Users/xuanwang/Desktop/O2_Cluster/2parse.csv")
parse <- parse[[1]]
codem <- read_csv("/Users/xuanwang/Desktop/O2_Cluster/2codem.csv")
code.m <- codem[[1]]
parhatm <- read_csv("/Users/xuanwang/Desktop/O2_Cluster/2parhatm.csv")
par.m <- parhatm[[1]]
each <- 5
...
index <- which(auc == 1111)
auchat <- NULL
for (j in 1:length(index)) {
  auchat <- cbind(auchat, matrix(auc[(index[j] + 1):(index[j] + 5 * each)], ncol = each))
}
index <- which(parse == 1111)
parsehat <- NULL
for (j in 1:length(index)) {
  parsehat <- cbind(parsehat, matrix(parse[(index[j] + 1):(index[j] + 16 * each)], ncol = each))
}
...
# result.tpr=data.frame(truth = (tprtrue), est = rowMeans(tpr.hat),bias = rowMeans(tpr.hat) - tprtrue,sd = apply(tpr.hat, 1, sd),
#                       se=apply(parsehat[12:16,],1,mean),
#                       cp=apply((tpr.hat+1.96*parsehat[12:16,]>tprtrue)*(tpr.hat-1.96*parsehat[12:16,]<tprtrue),1,mean)
#                       )
# rownames(result.tpr)=c("tpr1","tpr2","tpr3","tpr4","tpr5")
# print(round(result.tpr,3))
# result.fpr=data.frame(truth = (fprtrue), est = rowMeans(fpr.hat),bias = rowMeans(fpr.hat) - fprtrue,sd = apply(fpr.hat, 1, sd),
#                       se=apply(parsehat[17:21,],1,mean),
#                       cp=apply((fpr.hat+1.96*parsehat[17:21,]>fprtrue)*(fpr.hat-1.96*parsehat[17:21,]<fprtrue),1,mean)
#                       )
# rownames(result.fpr)=c("fpr1","fpr2","fpr3","fpr4","fpr5")
# print(round(result.fpr,3))

# set2
auctrue <- c(0.790, 0.830, 0.869, 0.902, 0.929)
# 0.789 0.832 0.871 0.904 0.930
result.auc <- data.frame(
  truth = (auctrue), est = rowMeans(auchat), bias = rowMeans(auchat) - auctrue, sd = apply(auchat, 1, sd),
  se = apply(parsehat[12:16, ], 1, mean),
  cp = apply((auchat + 1.96 * parsehat[12:16, ] > auctrue) * (auchat - 1.96 * parsehat[12:16, ] < auctrue), 1, mean)
)
rownames(result.auc) <- c("auc_1", "auc_2", "auc_3", "auc_4", "auc_5")
print(round(result.auc, 3))


all <- rbind(result, result.auc)
library(xtable)
